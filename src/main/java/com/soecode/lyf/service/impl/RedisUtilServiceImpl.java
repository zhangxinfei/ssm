...
public class RedisUtilServiceImpl implements RedisUtilService {
    ...
    public void del(String ... key){
        if(key!=null&&key.length>0){
            if(key.length==1){
                redisTemplate.delete(redisKey+key[0]);
            }else{
                redisTemplate.delete(CollectionUtils.arrayToList(redisKey+key));
            }
        }
    }
    ...
    public Object get(String key){
        return key==null?null:redisTemplate.opsForValue().get(redisKey+key);
    }
